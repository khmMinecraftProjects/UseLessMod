...
public class RenderView extends EntityRenderer {
	private boolean mouseMod(float par1) {
		ItemStack botas = new ItemStack(base.items.get(Datos.getI("idBotas")));
		ItemStack it = mc.thePlayer.inventory.armorInventory[0];
		if (it != null && it.isItemEqual(botas)
				&& ((Botas) it.getItem()).getRotation() != 0) {
			float rot = (float) ((Botas) it.getItem()).getRotation();

			this.mc.pointedEntityLiving = null;
			double d0 = (double) this.mc.playerController
					.getBlockReachDistance();

			float ySize = 1.4f * (rot / 180);
			...
			List list = this.mc.theWorld.getEntitiesWithinAABBExcludingEntity(
					...
					this.mc.renderViewEntity.boundingBox.addCoord(
							...
							vec31.zCoord * d0).expand((double) f1, (double) f1,
							(double) f1));
			double d2 = d1;
	...
	private void cameraUpdate() {
		...
		if (it != null && it.isItemEqual(botas)
			...
			float rot = (float) ((Botas) it.getItem()).getRotation();
			if (rot != 0) {
				GL11.glTranslatef(0.0F, ((float) 1.3 * (rot / 180)) - 0.1F,
						0.0F);
			}
	...
	public RenderView(Minecraft par1Minecraft) {
		super(par1Minecraft);
		this.mc = par1Minecraft;
		this.itemRenderer = new ItemRenderer(par1Minecraft);
		this.lightmapTexture = new DynamicTexture(16, 16);
		this.field_110922_T = par1Minecraft.func_110434_K().func_110578_a(
				"lightMap", this.lightmapTexture);
		this.lightmapColors = this.lightmapTexture.func_110565_c();
	}

	private static final ResourceLocation field_110924_q = new ResourceLocation(
			"textures/environment/rain.png");
	private static final ResourceLocation field_110923_r = new ResourceLocation(
	...
	public void updateRenderer() {
		...
		if (this.mc.gameSettings.smoothCamera) {
			...
			this.smoothCamFilterX = this.mouseFilterXAxis.smooth(
					this.smoothCamYaw, 0.05F * f1);
			this.smoothCamFilterY = this.mouseFilterYAxis.smooth(
					this.smoothCamPitch, 0.05F * f1);
	...
	private float getFOVModifier(float par1, boolean par2) {
		if (this.debugViewDirection > 0) {
			...
			int i = ActiveRenderInfo.getBlockIdAtEntityViewpoint(
					this.mc.theWorld, entityplayer, par1);

			if (i != 0 && Block.blocksList[i].blockMaterial == Material.water) {
				f1 = f1 * 60.0F / 70.0F;
			}

			return f1 + this.prevDebugCamFOV
					+ (this.debugCamFOV - this.prevDebugCamFOV) * par1;
		}
	}
	...
	private void setupViewBobbing(float par1) {
		if (this.mc.renderViewEntity instanceof EntityPlayer) {
			...
			GL11.glRotatef(MathHelper.sin(f2 * (float) Math.PI) * f3 * 3.0F,
					0.0F, 0.0F, 1.0F);
			GL11.glRotatef(
					Math.abs(MathHelper.cos(f2 * (float) Math.PI - 0.2F) * f3) * 5.0F,
					1.0F, 0.0F, 0.0F);
	...
	private void orientCamera(float par1) {
		...
		double d0 = entitylivingbase.prevPosX
				+ (entitylivingbase.posX - entitylivingbase.prevPosX)
				* (double) par1;
		double d1 = entitylivingbase.prevPosY
				+ (entitylivingbase.posY - entitylivingbase.prevPosY)
				* (double) par1 - (double) f1;
		double d2 = entitylivingbase.prevPosZ
				+ (entitylivingbase.posZ - entitylivingbase.prevPosZ)
				* (double) par1;
		GL11.glRotatef(this.prevCamRoll + (this.camRoll - this.prevCamRoll)
				* par1, 0.0F, 0.0F, 1.0F);
		...
		if (entitylivingbase.isPlayerSleeping()) {
			...
			double d3 = (double) (this.thirdPersonDistanceTemp + (this.thirdPersonDistance - this.thirdPersonDistanceTemp)
					* par1);
			float f2;
			float f3;

			if (this.mc.gameSettings.debugCamEnable) {
				f3 = this.prevDebugCamYaw
						+ (this.debugCamYaw - this.prevDebugCamYaw) * par1;
				f2 = this.prevDebugCamPitch
						+ (this.debugCamPitch - this.prevDebugCamPitch) * par1;
				...
				double d4 = (double) (-MathHelper.sin(f3 / 180.0F
						* (float) Math.PI) * MathHelper.cos(f2 / 180.0F
						* (float) Math.PI))
						* d3;
				double d5 = (double) (MathHelper.cos(f3 / 180.0F
						* (float) Math.PI) * MathHelper.cos(f2 / 180.0F
						* (float) Math.PI))
						* d3;
				double d6 = (double) (-MathHelper.sin(f2 / 180.0F
						* (float) Math.PI))
						* d3;
				...
				for (int l = 0; l < 8; ++l) {
					...
					MovingObjectPosition movingobjectposition = this.mc.theWorld
							.clip(this.mc.theWorld.getWorldVec3Pool()
									.getVecFromPool(d0 + (double) f4,
											d1 + (double) f5, d2 + (double) f6),
					...
					if (movingobjectposition != null) {
						double d7 = movingobjectposition.hitVec
								.distanceTo(this.mc.theWorld.getWorldVec3Pool()
										.getVecFromPool(d0, d1, d2));
				...
				GL11.glRotatef(entitylivingbase.rotationPitch - f2, 1.0F, 0.0F,
						0.0F);
				GL11.glRotatef(entitylivingbase.rotationYaw - f3, 0.0F, 1.0F,
						0.0F);
				GL11.glTranslatef(0.0F, 0.0F, (float) (-d3));
				GL11.glRotatef(f3 - entitylivingbase.rotationYaw, 0.0F, 1.0F,
						0.0F);
				GL11.glRotatef(f2 - entitylivingbase.rotationPitch, 1.0F, 0.0F,
						0.0F);
			}
		} else {
		...
		d0 = entitylivingbase.prevPosX
				+ (entitylivingbase.posX - entitylivingbase.prevPosX)
				* (double) par1;
		d1 = entitylivingbase.prevPosY
				+ (entitylivingbase.posY - entitylivingbase.prevPosY)
				* (double) par1 - (double) f1;
		d2 = entitylivingbase.prevPosZ
				+ (entitylivingbase.posZ - entitylivingbase.prevPosZ)
				* (double) par1;
	...
	private void setupCameraTransform(float par1, int par2) {
		...
		if (this.cameraZoom != 1.0D) {
			GL11.glTranslatef((float) this.cameraYaw,
					(float) (-this.cameraPitch), 0.0F);
		...
		if (this.debugViewDirection <= 0) {
			...
			Project.gluPerspective(this.getFOVModifier(par1, false),
					(float) this.mc.displayWidth
							/ (float) this.mc.displayHeight, 0.05F,
	...
	private void updateTorchFlicker() {
		this.torchFlickerDX = (float) ((double) this.torchFlickerDX + (Math
				.random() - Math.random()) * Math.random() * Math.random());
		this.torchFlickerDY = (float) ((double) this.torchFlickerDY + (Math
				.random() - Math.random()) * Math.random() * Math.random());
	...
	private void updateLightmap(float par1) {
		...
		if (worldclient != null) {
			for (int i = 0; i < 256; ++i) {
				...
				float f2 = worldclient.provider.lightBrightnessTable[i / 16]
						* f1;
				float f3 = worldclient.provider.lightBrightnessTable[i % 16]
				...
				float f4 = f2
						* (worldclient.getSunBrightness(1.0F) * 0.65F + 0.35F);
				float f5 = f2
						* (worldclient.getSunBrightness(1.0F) * 0.65F + 0.35F);
				...
				if (this.field_82831_U > 0.0F) {
					f11 = this.field_82832_V
							+ (this.field_82831_U - this.field_82832_V) * par1;
				...
				if (this.mc.thePlayer.isPotionActive(Potion.nightVision)) {
					f11 = this
							.getNightVisionBrightness(this.mc.thePlayer, par1);
					f12 = 1.0F / f8;
	...
	private float getNightVisionBrightness(EntityPlayer par1EntityPlayer,
			float par2) {
		int i = par1EntityPlayer.getActivePotionEffect(Potion.nightVision)
				.getDuration();
		return i > 200 ? 1.0F : 0.7F + MathHelper.sin(((float) i - par2)
		...
		if (!this.mc.skipRenderWorld) {
			...
			if (this.mc.theWorld != null) {
				...
				if (!this.mc.gameSettings.hideGUI
					...
					this.mc.ingameGUI.renderGameOverlay(par1,
							this.mc.currentScreen != null, k, l);
				}

				this.mc.mcProfiler.endSection();
			} else {
				GL11.glViewport(0, 0, this.mc.displayWidth,
						this.mc.displayHeight);
				GL11.glMatrixMode(GL11.GL_PROJECTION);
				GL11.glLoadIdentity();
				GL11.glMatrixMode(GL11.GL_MODELVIEW);
			...
			if (this.mc.currentScreen != null) {
				...
				try {
					...
					// new CallableMouseLocation(this, k, l));
					// crashreportcategory.addCrashSectionCallable("Screen size",
					// new CallableScreenSize(this, scaledresolution));
	...
	public void renderWorld(float par1, long par2) {
		...
		for (int j = 0; j < 2; ++j) {
			...
			ActiveRenderInfo.updateRenderInfo(this.mc.thePlayer,
			...
			if (j == 0) {
				...
				while (!this.mc.renderGlobal.updateRenderers(entitylivingbase,
						false) && par2 != 0L) {
			...
			if (this.debugViewDirection == 0) {
				...
				renderglobal.renderEntities(entitylivingbase.getPosition(par1),
						frustrum, par1);
				ForgeHooksClient.setRenderPass(0);
				/*
				 * Forge: Moved down this.enableLightmap((double)par1);
				 * this.mc.mcProfiler.endStartSection("litParticles");
				 * effectrenderer.renderLitParticles(entitylivingbase, par1);
				 * RenderHelper.disableStandardItemLighting(); this.setupFog(0,
				 * par1); this.mc.mcProfiler.endStartSection("particles");
				...
				if (this.mc.objectMouseOver != null
					...
					if (!ForgeHooksClient.onDrawBlockHighlight(renderglobal,
							entityplayer, mc.objectMouseOver, 0,
							entityplayer.inventory.getCurrentItem(), par1)) {
						renderglobal.drawSelectionBox(entityplayer,
								this.mc.objectMouseOver, 0, par1);
					}
			...
			if (this.mc.gameSettings.fancyGraphics) {
				...
				int l = renderglobal.sortAndRender(entitylivingbase, 1,
						(double) par1);
			...
			renderglobal.drawBlockDamageTexture(Tessellator.instance,
					entitylivingbase, par1);
			GL11.glDisable(GL11.GL_BLEND);
			...
			// Forge: Moved section from above, now particles are the last thing
			// to render.
			this.enableLightmap((double) par1);
			this.mc.mcProfiler.endStartSection("litParticles");
			effectrenderer.renderLitParticles(entitylivingbase, par1);
	...
	private void addRainParticles() {
		...
		if (f != 0.0F) {
			...
			for (int j1 = 0; j1 < i1; ++j1) {
				...
				BiomeGenBase biomegenbase = worldclient.getBiomeGenForCoords(
						k1, l1);
				...
				if (i2 <= j + b0 && i2 >= j - b0
					...
					if (j2 > 0) {
						if (Block.blocksList[j2].blockMaterial == Material.lava) {
							this.mc.effectRenderer.addEffect(new EntitySmokeFX(
									worldclient, (double) ((float) k1 + f1),
									(double) ((float) i2 + 0.1F)
											- Block.blocksList[j2]
													.getBlockBoundsMinY(),
									(double) ((float) l1 + f2), 0.0D, 0.0D,
									0.0D));
						} else {
							++l;
							...
							if (this.random.nextInt(l) == 0) {
								...
								d1 = (double) ((float) i2 + 0.1F)
										- Block.blocksList[j2]
												.getBlockBoundsMinY();
								d2 = (double) ((float) l1 + f2);
							}

							this.mc.effectRenderer.addEffect(new EntityRainFX(
									worldclient, (double) ((float) k1 + f1),
									(double) ((float) i2 + 0.1F)
											- Block.blocksList[j2]
													.getBlockBoundsMinY(),
			...
			if (l > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
				...
				if (d1 > entitylivingbase.posY + 1.0D
					...
					this.mc.theWorld.playSound(d0, d1, d2,
		...
		if (f1 > 0.0F) {
			...
			for (int k1 = i1 - b0; k1 <= i1 + b0; ++k1) {
				for (int l1 = k - b0; l1 <= k + b0; ++l1) {
					...
					BiomeGenBase biomegenbase = worldclient
							.getBiomeGenForCoords(l1, k1);
					...
					if (biomegenbase.canSpawnLightningBolt()
						...
						if (k2 != l2) {
							this.random
									.setSeed((long) (l1 * l1 * 3121 + l1
											* 45238971 ^ k1 * k1 * 418711 + k1
											* 13761));
							...
							if (worldclient.getWorldChunkManager()
									.getTemperatureAtHeight(f9, j2) >= 0.15F) {
								if (b1 != 0) {
									if (b1 >= 0) {
										tessellator.draw();
									}
								...
								f10 = ((float) (this.rendererUpdateCount + l1
										* l1 * 3121 + l1 * 45238971 + k1 * k1
										* 418711 + k1 * 13761 & 31) + par1)
										/ 32.0F
										* (3.0F + this.random.nextFloat());
								double d4 = (double) ((float) l1 + 0.5F)
										- entitylivingbase.posX;
								d3 = (double) ((float) k1 + 0.5F)
										- entitylivingbase.posZ;
								float f11 = MathHelper.sqrt_double(d4 * d4 + d3
										* d3)
										/ (float) b0;
								float f12 = 1.0F;
								tessellator.setBrightness(worldclient
										.getLightBrightnessForSkyBlocks(l1, i3,
												k1, 0));
								tessellator
										.setColorRGBA_F(
												f12,
												f12,
												f12,
												((1.0F - f11 * f11) * 0.5F + 0.5F)
														* f1);
								tessellator.setTranslation(-d0 * 1.0D,
										-d1 * 1.0D, -d2 * 1.0D);
								tessellator.addVertexWithUV(
										(double) ((float) l1 - f6) + 0.5D,
										(double) k2,
										(double) ((float) k1 - f7) + 0.5D,
										(double) (0.0F * f8),
										(double) ((float) k2 * f8 / 4.0F + f10
												* f8));
								tessellator.addVertexWithUV(
										(double) ((float) l1 + f6) + 0.5D,
										(double) k2,
								...
								float f13 = this.random.nextFloat() + f5
										* 0.01F
										* (float) this.random.nextGaussian();
								float f14 = this.random.nextFloat() + f5
										* (float) this.random.nextGaussian()
										* 0.001F;
								d3 = (double) ((float) l1 + 0.5F)
										- entitylivingbase.posX;
								double d5 = (double) ((float) k1 + 0.5F)
										- entitylivingbase.posZ;
								float f15 = MathHelper.sqrt_double(d3 * d3 + d5
										* d5)
										/ (float) b0;
								float f16 = 1.0F;
								tessellator.setBrightness((worldclient
	...
	public void setupOverlayRendering() {
		...
		GL11.glOrtho(0.0D, scaledresolution.getScaledWidth_double(),
				scaledresolution.getScaledHeight_double(), 0.0D, 1000.0D,
				3000.0D);
		GL11.glMatrixMode(GL11.GL_MODELVIEW);
		GL11.glLoadIdentity();
	...
	private void updateFogColor(float par1) {
		...
		if (this.mc.gameSettings.renderDistance < 2) {
			Vec3 vec32 = MathHelper.sin(worldclient
					.getCelestialAngleRadians(par1)) > 0.0F ? worldclient
					.getWorldVec3Pool().getVecFromPool(-1.0D, 0.0D, 0.0D)
					: worldclient.getWorldVec3Pool().getVecFromPool(1.0D, 0.0D,
							0.0D);
			...
			if (f5 > 0.0F) {
				float[] afloat = worldclient.provider.calcSunriseSunsetColors(
						worldclient.getCelestialAngle(par1), par1);

				if (afloat != null) {
					f5 *= afloat[3];
					this.fogColorRed = this.fogColorRed * (1.0F - f5)
							+ afloat[0] * f5;
					this.fogColorGreen = this.fogColorGreen * (1.0F - f5)
							+ afloat[1] * f5;
					this.fogColorBlue = this.fogColorBlue * (1.0F - f5)
							+ afloat[2] * f5;
				}
			}
		}
		...
		int i = ActiveRenderInfo.getBlockIdAtEntityViewpoint(this.mc.theWorld,
				entitylivingbase, par1);
		float f8;
		...
		double d0 = (entitylivingbase.lastTickPosY + (entitylivingbase.posY - entitylivingbase.lastTickPosY)
				* (double) par1)
		...
		if (this.field_82831_U > 0.0F) {
			f9 = this.field_82832_V + (this.field_82831_U - this.field_82832_V)
					* par1;
			this.fogColorRed = this.fogColorRed * (1.0F - f9)
					+ this.fogColorRed * 0.7F * f9;
			this.fogColorGreen = this.fogColorGreen * (1.0F - f9)
					+ this.fogColorGreen * 0.6F * f9;
			this.fogColorBlue = this.fogColorBlue * (1.0F - f9)
					+ this.fogColorBlue * 0.6F * f9;
		}
		...
		if (entitylivingbase.isPotionActive(Potion.nightVision)) {
			...
			this.fogColorRed = this.fogColorRed * (1.0F - f9)
					+ this.fogColorRed * f10 * f9;
			this.fogColorGreen = this.fogColorGreen * (1.0F - f9)
					+ this.fogColorGreen * f10 * f9;
			this.fogColorBlue = this.fogColorBlue * (1.0F - f9)
					+ this.fogColorBlue * f10 * f9;
		}
		...
		GL11.glClearColor(this.fogColorRed, this.fogColorGreen,
				this.fogColorBlue, 0.0F);
	}
	...
	private void setupFog(int par1, float par2) {
		...
		if (par1 == 999) {
			GL11.glFog(GL11.GL_FOG_COLOR,
					this.setFogColorBuffer(0.0F, 0.0F, 0.0F, 1.0F));
			...
			GL11.glFog(GL11.GL_FOG_COLOR, this.setFogColorBuffer(
					this.fogColorRed, this.fogColorGreen, this.fogColorBlue,
					1.0F));
			GL11.glNormal3f(0.0F, -1.0F, 0.0F);
			GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
			int j = ActiveRenderInfo.getBlockIdAtEntityViewpoint(
					this.mc.theWorld, entitylivingbase, par2);
			float f1;

			if (entitylivingbase.isPotionActive(Potion.blindness)) {
				f1 = 5.0F;
				int k = entitylivingbase
						.getActivePotionEffect(Potion.blindness).getDuration();

				if (k < 20) {
					f1 = 5.0F + (this.farPlaneDistance - 5.0F)
							* (1.0F - (float) k / 20.0F);
				}
				...
				GL11.glFogf(GL11.GL_FOG_DENSITY, 0.1F);
					...
					&& Block.blocksList[j].blockMaterial == Material.water) {
				GL11.glFogi(GL11.GL_FOG_MODE, GL11.GL_EXP);
				...
				if (entitylivingbase.isPotionActive(Potion.waterBreathing)) {
					...
					GL11.glFogf(GL11.GL_FOG_DENSITY,
							0.1F - (float) EnchantmentHelper
									.getRespiration(entitylivingbase) * 0.03F);
				}
			} else if (j > 0
					&& Block.blocksList[j].blockMaterial == Material.lava) {
				GL11.glFogi(GL11.GL_FOG_MODE, GL11.GL_EXP);
				...
				if (this.mc.theWorld.provider.getWorldHasVoidParticles()
						&& !flag) {
					double d0 = (double) ((entitylivingbase
							.getBrightnessForRender(par2) & 15728640) >> 20)
							/ 16.0D
							+ (entitylivingbase.lastTickPosY
									+ (entitylivingbase.posY - entitylivingbase.lastTickPosY)
									* (double) par2 + 4.0D) / 32.0D;
				...
				if (this.mc.theWorld.provider.doesXZShowFog(
						(int) entitylivingbase.posX,
						(int) entitylivingbase.posZ)) {
