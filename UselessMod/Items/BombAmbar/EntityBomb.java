...
public class EntityBomb extends EntityThrowable {
	...
	int ticks = 0, tiempo = 20, radio = 20, max = tiempo * radio, idA = Datos
			.getI("idAmbar");
	...
	public EntityBomb(World par1World, EntityLivingBase par2EntityLivingBase,
		...
		tiempo = Datos.getI("Ambar_Tiempo");
		radio = Datos.getI("Ambar_Radio");
		cubo = Datos.getB("Ambar_Cubo");
	}
	...
	public void Ambarizar(int x, int y, int z) {
		if (!this.worldObj.isRemote) {
			if (this.worldObj.getBlockId((int) (posX + x), (int) (posY + y),
					(int) (posZ + z)) == 0) {
				this.worldObj.setBlock((int) (posX + x), (int) (posY + y),
						(int) (posZ + z), idA);
			}
		}
	}
	...
	public void spawnParticles(int x, int y, int z) {
		for (int i = -1; i < 2; i += 2) {
			for (int o = 0; o < 5; o++) {
				...
				this.worldObj.spawnParticle("smoke", posX + x + parX + i, posY
						+ y + parY + i, posZ + z + parZ + i, 0.0D, 0.0D, 0.0D);
			}
		}
	}
