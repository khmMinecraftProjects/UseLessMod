...
public class PiesJesus extends ItemArmor {
	...
	public PiesJesus(int par1) {
		super(par1, EnumArmorMaterial.DIAMOND, base.proxy.addArmors("pies"), 3);

		this.damageReduceAmount = new int[] { 0, 0, Datos.getI("Pies_Armor"), 0 };

		this.empuje = Datos.getI("Pies_Empuje");
	...
	public void onArmorTickUpdate(World world, EntityPlayer player,
			ItemStack itemStack) {
		int id = world.getBlockId((int) player.posX, (int) (player.posY),
				(int) player.posZ);
		if (player.isInWater() || id == 10 || id == 11) {
			player.moveEntity(0, empuje, 0);

		} else if (Ball.esLiquido(world, (int) player.posX,
				(int) (player.posY - 1.9), (int) player.posZ)
